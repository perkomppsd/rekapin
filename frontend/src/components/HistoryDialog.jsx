...
function ActionBadge({ action }) {
  const cfg = {
    created: { cls: "bg-emerald-500/10 text-emerald-700 dark:text-emerald-300 border-emerald-500/20", icon: Plus, label: "Dibuat" },
...
function ChangeRow({ change }) {
  ...
  return (
    <div className="text-sm flex flex-wrap items-center gap-2 text-slate-600 dark:text-slate-300">
      ...
      <span className="text-slate-900 dark:text-slate-100 max-w-[320px] truncate">{String(change.new || "—")}</span>
    </div>
  );
}
...
export default function HistoryDialog({ open, onOpenChange, candidate }) {
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-2xl bg-white dark:bg-slate-900 border-slate-200 dark:border-slate-800 text-slate-900 dark:text-slate-50 max-h-[85vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle className="font-display text-2xl flex items-center gap-2">
            <History className="w-5 h-5 text-indigo-600 dark:text-indigo-400" /> Timeline Kandidat
          </DialogTitle>
          <DialogDescription className="text-slate-500 dark:text-slate-400">
            Semua perubahan data untuk <span className="text-slate-800 dark:text-slate-200 font-medium">{candidate?.nama}</span>.
          </DialogDescription>
        </DialogHeader>

        {loading ? (
          <div className="text-slate-500 dark:text-slate-400 text-sm py-8 text-center">Memuat...</div>
        ) : entries.length === 0 ? (
          <div className="text-slate-500 dark:text-slate-400 text-sm py-8 text-center border border-dashed border-slate-200 dark:border-slate-800 rounded-lg">
            Belum ada riwayat perubahan.
          </div>
        ) : (
          <ol className="relative border-l border-slate-200 dark:border-slate-800 pl-6 space-y-6" data-testid="history-list">
            {entries.map((e) => (
              <li key={e.id} className="relative" data-testid={`history-entry-${e.id}`}>
                ...
                <div className="flex items-center gap-2 flex-wrap">
                  ...
                  <span className="text-slate-500 dark:text-slate-400 text-xs">{formatTs(e.changed_at)}</span>
