    ...
    <div className={T.page}>
      <header className={T.nav}>
        <div className={`${T.containerNarrow} h-16 flex items-center justify-between`}>
          <Link to="/dashboard" className="flex items-center gap-2 text-slate-300 hover:text-slate-50">
        ...
        <div>
          {badge ? (
            <div className="inline-flex items-center gap-2 px-2.5 py-1 rounded-full border border-indigo-500/30 bg-indigo-500/10 mb-3">
              {BadgeIcon ? <BadgeIcon className="w-3 h-3 text-indigo-300" /> : null}
              <span className="text-indigo-200 text-[10px] tracking-[0.2em] uppercase">{badge}</span>
            </div>
          ) : null}
