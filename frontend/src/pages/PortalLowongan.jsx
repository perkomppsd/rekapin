    ...
    <PortalShell>
      <div className="mb-8">
        <div className="inline-flex items-center gap-2 px-2.5 py-1 rounded-full border border-indigo-500/30 bg-indigo-500/10 mb-3">
          ...
          <span className="text-indigo-200 text-[10px] tracking-[0.2em] uppercase">Lowongan Terbuka</span>
        </div>
      ...
      {status === "loading" && <div className="text-slate-400 py-12 text-center">Memuat lowongan...</div>}
      {status === "error" && (
        <div className={`${T.panelSubtle} p-8 text-center text-slate-300`}>{pesan}</div>
      )}
      {status === "ready" && items.length === 0 && (
        <div className={`${T.panelSubtle} p-12 text-center`}>
          <p className="text-slate-300">Belum ada lowongan yang dibuka saat ini.</p>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        {items.map((job) => (
          <Link key={job.slug} to={`/lowongan/${job.slug}`}
            ...
            className={`${T.panel} p-5 block hover:border-indigo-500/40 transition-colors`}>
            <div className="flex items-start justify-between gap-3">
              <h2 className="font-display text-lg font-semibold text-slate-50">{job.judul}</h2>
            ...
            {job.deskripsi && (
              <p className="text-slate-400 text-sm mt-3 line-clamp-3">{job.deskripsi}</p>
            )}
          </Link>
        ))}
      </div>
